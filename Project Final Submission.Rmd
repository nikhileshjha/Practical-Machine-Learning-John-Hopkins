...
The Accuracy on the training data from the recursive tree is 66.16% and on the cross validation set it 66.17%.
The Accuracy on the training data from Linear Discriminant Analysis is 100% and on the cross validation data it 99.98%.
Like the LDA Analysis, the Random Forest model has an accuracy of 100% on the training data and 99.98% accuracy on cross validation data.
...
```{r Result, echo = FALSE}
options(warn=-1)
# Running a Linear Discriminant Analysis model

model_lda <- train(classe ~ ., method = "lda",trControl = control, data = red_training)

```
# Final Prediction
```{r print}
predict(model_lda,red_test)
```
All the 20 entries in the final data set correspond to correct execution of exercise in both the LDA and Random Forest models.